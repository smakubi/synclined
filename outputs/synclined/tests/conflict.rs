...
fn conflicting_proposal_is_flagged_without_changing_confirmed_state() {
    ...
    let confirmed = engine
        .propose(task.id, RecordType::Decision, "Keep the timeout at ten seconds", "developer")
        .unwrap();
    engine.review(confirmed.id, ProposalOutcome::Accept).unwrap();

    let proposal = engine
        .propose(task.id, RecordType::Decision, "Raise the timeout to thirty seconds", "cloud-code")
        .unwrap();

    assert!(proposal.is_conflict);
    assert_eq!(proposal.conflicts_with, Some(confirmed.id));
    assert_eq!(engine.resume(task.id).unwrap().confirmed[0].content, "Keep the timeout at ten seconds");
...
fn open_questions_do_not_create_possible_conflicts() {
    ...
    let confirmed = engine
        .propose(task.id, RecordType::OpenQuestion, "Is ten seconds intended?", "developer")
        .unwrap();
    engine.review(confirmed.id, ProposalOutcome::Accept).unwrap();

    let proposal = engine
        .propose(task.id, RecordType::OpenQuestion, "Should we measure failures?", "cloud-code")
        .unwrap();
