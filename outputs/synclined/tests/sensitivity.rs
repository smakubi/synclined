...
fn explicit_sensitive_markers_require_release_review_case_insensitively() {
    ...
    for content in [
        ...
        let proposal = engine
            .propose(task.id, RecordType::Change, content, "cloud-code")
            .unwrap();
        assert_eq!(
            proposal.sensitive_release_state,
...
fn routine_content_does_not_require_release_review_and_state_persists() {
    ...
    assert_eq!(
        reopened.pending(task.id).unwrap()[0].sensitive_release_state,
        SensitiveReleaseState::NotRequired
    );
}
...
fn unrecognized_persisted_sensitive_release_state_fails_closed() {
    ...
    let task = engine
        ...
        .unwrap();
    let proposal = engine
        .propose(task.id, RecordType::Change, "Routine change", "cloud-code")
        .unwrap();
    drop(engine);
