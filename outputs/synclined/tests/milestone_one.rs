...
fn persists_accepted_and_rejected_proposals_for_an_independent_reader() {
    ...
    let accepted = board.propose(task.id, RecordType::OpenQuestion, "Is ten seconds intended?", "agent-a").unwrap();
    board.review(accepted.id, ProposalOutcome::Accept).unwrap();

    let rejected = board.propose(task.id, RecordType::Change, "Set timeout to thirty seconds", "agent-a").unwrap();
    ...
    assert_eq!(snapshot.rejected[0].content, "Set timeout to thirty seconds");
}
