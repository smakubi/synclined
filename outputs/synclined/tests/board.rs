...
fn board_view_shows_goal_and_pending_proposals_then_reviews_through_engine() {
    ...
    let proposal = engine.propose(task.id, RecordType::Change, "Raise timeout", "cloud-code").unwrap();
    ...
    assert!(synclined_core::BoardView::load(&api, task.id).unwrap().pending.is_empty());
    assert_eq!(engine.resume(task.id).unwrap().confirmed.len(), 1);
    drop(api);

    let rejected = engine.propose(task.id, RecordType::OpenQuestion, "Is it intended?", "cloud-code").unwrap();
    let api = TaskAccessApi::new(&engine);
    api.reject(rejected.id).unwrap();
    assert_eq!(engine.resume(task.id).unwrap().rejected.len(), 1);
    drop(api);

    let edited = engine.propose(task.id, RecordType::Change, "Raise timeout", "cloud-code").unwrap();
    let api = TaskAccessApi::new(&engine);
    api.edit_and_accept(edited.id, "Raise timeout after test evidence").unwrap();
    assert_eq!(engine.resume(task.id).unwrap().confirmed[1].content, "Raise timeout after test evidence");
}
