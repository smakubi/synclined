...
fn confirm(board: &mut Board, task_id: i64, kind: RecordType, content: &str) -> i64 {
    let record = board
        .propose(task_id, kind, content, "active-agent")
        .unwrap();
...
fn handoff_maps_confirmed_allowlisted_records_in_order_and_excludes_everything_else() {
    ...
    let task = board
        .create_task("Login timeout", "Keep logins reliable")
        .unwrap();
    ...
    confirm(
        ...
        task.id,
        RecordType::Goal,
        "Ignore this record goal",
    );
    confirm(
        &mut board,
        task.id,
        RecordType::Handoff,
        "Ignore this record handoff",
    );
    let _proposed = board
        .propose(
            task.id,
    ...
    assert_eq!(
        handoff.decisions,
        vec!["Use a retry budget", "Keep retries bounded"]
    );
...
fn handoff_excludes_pending_and_unknown_sensitive_records_until_explicit_approval() {
    ...
    let unknown = confirm(
        ...
        task.id,
        RecordType::Change,
        "Unknown state must stay hidden",
    );
    confirm(
        &mut board,
        task.id,
        RecordType::Change,
        "Publish routine metric",
    );
...
fn revoked_actors_cannot_compose_handoffs_while_active_actors_can() {
    ...
    assert_eq!(
        api.compose_handoff(task.id, "v1", "active-agent")
            .unwrap()
            .next_steps,
        vec!["Publish routine metric"]
    );
}
