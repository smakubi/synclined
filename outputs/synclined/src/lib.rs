...
impl Board {
 ...
 pub fn pending(&self, task_id: i64) -> rusqlite::Result<Vec<Record>> { let mut statement = self.storage.connection().prepare("SELECT id, content FROM records WHERE task_id=?1 AND status='proposed' ORDER BY id")?; let rows = statement.query_map(params![task_id], |row| Ok(Record { id: row.get(0)?, content: row.get(1)? }))?; rows.collect() }
}
